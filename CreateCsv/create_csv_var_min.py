...
def get_controlled_json_time(version, app, feature, json_commands, reason):
    ...
    if (app == "Youtube" and feature == "play video"):
        times["search video page"] = time_from_commands(
            commands, pressKeyPos=1)
        ...
        times["search channel page"] = time_from_commands(
            commands, pressKeyPos=1)
        ...
        times[feature] = time_from_commands(
            commands, pressKeyPos=1, isElementDisplayed=False, app="GoogleMaps")

    elif (app == "GoogleNews" and feature == "search news"):
        # print(app, feature, version)
        # print(json.dumps(commands, indent=2))
        times[feature] = time_from_commands(commands, clickPos=1)

    elif (app == "Telegram" and feature == "send message"):
        if (int(ANDROID_OS) == 8):
            times[feature] = time_from_commands(commands, clickPos=7)
        else:
        ...
        if (int(ANDROID_OS) == 8):
            if int(version[-1]) < 3:
                if (reason == "add to cart clicked"):
                    times["search product result"] = time_from_commands(
                        commands, clickPos=3)
                    times["product profile"] = time_from_commands(
                        commands, clickPos=5)
                    times["add to cart"] = time_from_commands(
                        commands, clickPos=6)
                    times["go to cart"] = time_from_commands(
                        commands, clickPos=7)
                    times["remove from cart"] = time_from_commands(
                        commands, clickPos=8)
        ...
        times["search results page"] = time_from_commands(
            commands, pressKeyPos=1, isElementDisplayed=False, app="Facebook")
        times["person profile page"] = time_from_commands(
            commands, clickPos=3, isElementDisplayed=False, app="Facebook")
        ...
        if int(version[-1]) < 2:
            ...
            times["post in a group"] = time_from_commands(commands, clickPos=9)
...
def get_time_from_json(app, feature, json_commands, reason):
    ...
    if (app == "Youtube" and feature == "play video"):
        ...
        times["search video page"] = time_from_commands(
            commands, app="Hotstar", sendKeysPos=1)
        times["play video page"] = time_from_commands(commands, clickPos=2)
        ...
        times["product profile page"] = time_from_commands(
            commands, clickPos=3)
        if (reason == "add to cart clicked"):
            times["add to cart clicked"] = time_from_commands(
                commands, clickPos=4)
            times["go to cart clicked"] = time_from_commands(
                commands, clickPos=6)
            times["remove from cart clicked"] = time_from_commands(
                commands, clickPos=8)
        else:
            times["go to cart clicked"] = time_from_commands(
                commands, clickPos=4)
        ...
        times["sending message"] = time_from_commands(
            commands, clickPos=2, app="Whatsapp")

    elif (app == "GoogleMaps" and feature == "search place"):
        times["searching place"] = time_from_commands(
            commands, pressKeyPos=1, isElementDisplayed=False, app="GoogleMaps")
        ...
        times["opening news page"] = time_from_commands(
            commands, clickPos=2, isElementDisplayed=False, app="GoogleNews")
        ...
        times[feature] = time_from_commands(
            commands, clickPos=6, isElementDisplayed=False, app="Facebook")
...
def assign_test_name(app, feature):
    ...
    elif feature == "playVideo" or feature == "play test":
        feature = "play video"
...
def getCollectionNames():
    ...
    if (VTYPE == "compare_versions"):
        for versionId in VERSION_ID:
            collectionName = DEVICE_NAME + "-os" + \
                str(ANDROID_OS) + "_v" + str(versionId)
            collectionNames += [collectionName]
    else:
        collectionName = DEVICE_NAME + "-os" + \
            str(ANDROID_OS) + "_v" + str(VERSION_ID)
        collectionNames += [collectionName]
    ...
    for doc in MONGO_COLLECTION.find():
        if (u'status' in doc.keys() and doc[u'status']):
            ...
            for key in timesObj.keys():
                ...
                data[app][key]['avg'] = (
                    avg * (count-1) + timesObj[key]) / count
    ...
    for app in apps:
        for test in DATA[app].keys():
            ...
            for feature in DATA[app][test].keys():
                ...
                cnt += 1
                ax.boxplot(y_axis_data, labels=versionLabels,
            ...
            output_path = CONTROLLED_EXP_FOLDER + \
                features_to_save + "/" + app + "_" + test + ".png"
            # plt.show()
...
def write_to_csv():
    ...
    elif (len(sys.argv) > 2 and sys.argv[2] == "trx-rcd-bytes"):
        Collections = [sys.argv[2]]
        
    # elif (len(sys.argv) > 2 and sys.argv[2] == "arani@iiitd.ac.in"):
    elif (len(sys.argv) > 2 and sys.argv[2] == "rahul20117@iiitd.ac.in"):
        Collections = [sys.argv[2]]
        print(Collections)
    else:
    ...
    if (sys.argv[1] in ["student-app-count-mc", "student-app-count-wn", "all"]):
        fields = ["student", "location", "android_version", "ram",
                  "distance_from_nearest_datacenter", "wifi_speed", "mobile_data_speed"]
        for app in APPS:
    ...
    for col in Collections:
            ...
            if u'Location' in doc.keys():
                ...
                if u'json' in doc.keys():
                    ...
                    if (sys.argv[1] == "controlled"):
                        ...
                        feature_performance = get_controlled_json_time(
                            STUDENT[-2:], app, feature, json_commands, doc[u'reason'])

                    else:
                        row = [LOCATION, time, CONNECTION_TYPE, app]
                        feature_performance = get_time_from_json(
                            app, feature, json_commands, doc[u'reason'])
                    ...
                    for f in feature_performance.keys():
                        row.extend([f, feature_performance[f]])

                    rx, tx, n_tcp, n_udp = get_data_consumed_single_app(
                        app, doc[u'data_before'], doc[u'data_after'], doc)

                    if (sys.argv[1] == "controlled"):
                        row.extend(
                            ["rx", rx, "tx", tx, "n_tcp", n_tcp, "n_udp", n_udp])
                            ...
                            if 'rt' in ping_row.keys() and ping_row['rt'] != '' and ping_row['t'] != '':
                                ...
                                if (ping_row['t'][:10].isnumeric()):
                                    ...
                                    if (len(ping) == 3):
                                        ...
                                        if (date[0] <= 31):
                                            dt = datetime.datetime(
                                                date[2], date[1], date[0], time[0], time[1], time[2])
                                        else:
                                            dt = datetime.datetime(
                                ...
                                if (ping_row['t'] in ping_data[website].keys()):
                                    ping_data[website][ping_row['t']].append(
                                        ping_row['rt'])
                                else:
                                    ping_data[website][ping_row['t']] = [
                                        ping_row['rt']]
        ...
        if (sys.argv[1] in ["student-app-count-mc", "student-app-count-wn", "all"]):
            ...
            if (flag):
                ...
                data_row = [STUDENT, LOCATION, STUDENT_DETAILS[STUDENT]["android_os"],
                            STUDENT_DETAILS[STUDENT]["ram"], STUDENT_DETAILS[STUDENT]["distance"]]
                for app in APPS:
                    for daytime in daytimes:
                        for conn_type in CONN_TYPES:
                            key = app + "_" + daytime + "_" + conn_type
        ...
        if (sys.argv[1].split("-")[0] in ["pingGoogle_graphs", "find_correlation", "jumbo", "all"]):
            ...
            for i in range(len(rows)):
                ...
                ping_variance_google = np.var(ping_range["google"])
                ping_variance_amazon = np.var(ping_range["amazon"])
                ping_variance_mobikwik = np.var(ping_range["mobikwik"])

                ping_min_google = np.min(ping_range["google"])
                ping_min_amazon = np.min(ping_range["amazon"])
                ping_min_mobikwik = np.min(ping_range["mobikwik"])
                ...
                for j in range(4, len(row), 2):
                    ...
                    correlation_data = [cor_key, STUDENT, android_os, distance / MAX_STUDENT_DISTANCE, ram,
                                        latency, time, net_type, ping_median_google, ping_median_amazon, ping_median_mobikwik,
                                        ping_variance_google, ping_variance_amazon, ping_variance_mobikwik, ping_min_google, ping_min_amazon, ping_min_mobikwik]
    ...
    if ("controlled" in sys.argv[1]):
        ...
        for i in range(len(controlled_rows)):
            ...
            for j in range(4, len(row), 2):
                ...
                if (test_name not in controlled_data[app].keys()):
                    ...
                    controlled_data[app][test_name]["latency"] = {
                        "v1": [], "v2": [], "v3": []}
                    controlled_data[app][test_name]["rx"] = {
                        "v1": [], "v2": [], "v3": []}
                    controlled_data[app][test_name]["tx"] = {
                        "v1": [], "v2": [], "v3": []}
                    controlled_data[app][test_name]["n_udp"] = {
                        "v1": [], "v2": [], "v3": []}
                    controlled_data[app][test_name]["n_tcp"] = {
                        "v1": [], "v2": [], "v3": []}
                    controlled_data[app][test_name]["ping_median_google"] = {
                        "v1": [], "v2": [], "v3": []}
                    controlled_data[app][test_name]["ping_median_amazon"] = {
                        "v1": [], "v2": [], "v3": []}
                    controlled_data[app][test_name]["ping_median_mobikwik"] = {
                        "v1": [], "v2": [], "v3": []}

                latency = row[j+1]

                r = [app, version]
                r.extend([test_name, latency, rx, tx, n_tcp, n_udp,
                          ping_median_google, ping_median_amazon, ping_median_mobikwik,
                          ping_variance_google, ping_variance_amazon, ping_variance_mobikwik, ping_min_google, 
                          ping_min_amazon, ping_min_mobikwik])
                ...
                controlled_data[app][test_name]["ping_median_google"][version] += [
                    ping_median_google]
                controlled_data[app][test_name]["ping_median_amazon"][version] += [
                    ping_median_amazon]
                controlled_data[app][test_name]["ping_median_mobikwik"][version] += [
                    ping_median_mobikwik]
                controlled_data[app][test_name]["ping_variance_google"][version] += [
                    ping_variance_google]
                controlled_data[app][test_name]["ping_variance_amazon"][version] += [
                    ping_variance_amazon]
                controlled_data[app][test_name]["ping_variance_mobikwik"][version] += [
                    ping_variance_mobikwik]

                controlled_data[app][test_name]["ping_min_google"][version] += [
                    ping_min_google]
                controlled_data[app][test_name]["ping_min_amazon"][version] += [
                    ping_min_amazon]
                controlled_data[app][test_name]["ping_min_mobikwik"][version] += [
                    ping_min_mobikwik]
        ...
        with open(filename, 'w', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerow(['APP', 'VERSION', 'FEATURE_NAME', 'LATENCY', 'RX_BYTES (Mb)', 'TX_BYTES (Mb)',
                                '#TCP_CONNECTIONS', '#UDP_CONNECTIONS', 'PNIG_MEDIAN_GOOGLE', 'PING_MEDIAN_AMAZON', 'PING_MEDIAN_MOBIKWIK'])
    ...
    if (sys.argv[1].split("-")[0] == "pingGoogle_graphs" or sys.argv[1].split("-")[0] == "all"):
        for key in ping_graphs_data:
            ...
            y_ticks_set = [y_tick for y_tick in range(
                0, max(y_values) + 500, 500)]
            x_ticks_set = []
            x_ticks_label = []
    ...
    if (sys.argv[1].split("-")[0] == "jumbo" or sys.argv[1].split("-")[0] == "all"):
        ...
        fields = ["FEATURE", "STUDENT", "ANDROID", "DISTANCE", "RAM", "LATENCY", "TIME_OF_DAY",
                  "NET_TYPE", "PING_MEDIAN_GOOGLE", "PING_MEDIAN_AMAZON", "PING_MEDIAN_MOBIKWIK",
                  "PING_VARIANCE_GOOGLE", "PING_VARIANCE_AMAZON", "PING_VARIANCE_MOBIKWIK", 
                  "PING_MIN_GOOGLE", "PING_MIN_AMAZON", "PING_MIN_MOBIKWIK"]
...
if __name__ == "__main__":
    ...
    print(sys.argv[1])
    # print(DEVICE_NAME)
    # print(STUDENTS_FOLDER)
    write_to_csv()
    total_tests = {"youtube": 2, "linkedin": 3, "hotstar": 2, "gmaps": 1, "gnews": 1, "amazon": 1,
                   "flipkart": 1, "facebook": 2, "paytm": 1, "mobikwik": 1, "telegram": 1, "whatsapp": 1}
