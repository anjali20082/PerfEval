		...
		if (int(ANDROID_OS) == 8):
			if int(version[-1]) < 3:
				if (reason == "add to cart clicked"):
					times["search product result"] = time_from_commands(commands, clickPos=3)
					times["product profile"] = time_from_commands(commands, clickPos=5)
					times["add to cart"] = time_from_commands(commands, clickPos=6)
					times["go to cart"] = time_from_commands(commands, clickPos=7)
					times["remove from cart"] = time_from_commands(commands, clickPos=8)

					# times[feature] = getFeatureTotalTime(times)
			else:
				if (reason == "add to cart clicked"):
					times["search product result"] = time_from_commands(commands, clickPos=5)
			...
			if int(version[-1]) < 3:
				if (reason == "add to cart clicked"):
					times["search product result"] = time_from_commands(commands, clickPos=3)
					times["product profile"] = time_from_commands(commands, clickPos=4)
					times["add to cart"] = time_from_commands(commands, clickPos=5)
					times["go to cart"] = time_from_commands(commands, clickPos=6)
					times["remove from cart"] = time_from_commands(commands, clickPos=7)

					# times[feature] = getFeatureTotalTime(times)
			else:
				if (reason == "add to cart clicked"):
					times["search product result"] = time_from_commands(commands, clickPos=5)
					times["product profile"] = time_from_commands(commands, clickPos=6)
					times["add to cart"] = time_from_commands(commands, clickPos=7)
					times["go to cart"] = time_from_commands(commands, clickPos=8)
					times["remove from cart"] = time_from_commands(commands, clickPos=9)
...
def write_to_csv():
	...
	elif (len(sys.argv) > 2 and sys.argv[2] == "location_test"):
		Collections = [sys.argv[2]]
	else:
	...
	for col in Collections:
		...
		for doc in MONGO_COLLECTION.find():	
			...
			if u'Location' in doc.keys():
				...
				if u'json' in doc.keys():
					...
					if (u'latitude' in doc):
						location_coord = True
						latitude = doc[u'latitude']
						longitude = doc[u'longitude']
						altitude = doc[u'altitude']
						# print(CONNECTION_TYPE)						
					...
					# print("before: ",app," ",feature)
					feature = assign_test_name(app, feature)
					# print("after: ",app," ",feature,"\n\n")
					...
					if (key not in student_app_feature_count):
						...
						student_app_feature_count[key] += 1
					# print(student_app_feature_count)
					...
					if (sys.argv[1] == "controlled"):
						row.extend(["rx", rx, "tx", tx, "n_tcp", n_tcp, "n_udp", n_udp])
						controlled_rows.append(row)						
						
					else:
						row.extend(["rx", rx, "tx", tx])
						row.extend([DAYTIME])
						row.extend([latitude, longitude, altitude])
						rows.append(row)
					# print(row)
		...
		# print("controlled: ",controlled_rows)
		...
		if (sys.argv[1].split("-")[0] in ["pingGoogle_graphs", "find_correlation", "jumbo", "all"]):
			...
			for i in range(len(rows)):
				...
				# if (len(ping_range["google"]) == 0 and len(ping_range["amazon"]) == 0 and len(ping_range["mobikwik"]) == 0):
				...
				for j in range(4, len(row), 2):
					...
					if location_coord:
						...
						correlation_data = [cor_key, STUDENT, android_os, distance / MAX_STUDENT_DISTANCE, ram,
                                            latency, time, net_type, ping_median_google, ping_median_amazon, ping_median_mobikwik, latitude, longitude, altitude]
					# print("test ", correlation_data)
					correlation_matrix += [correlation_data]
					
					
					ping_graphs_data[key].append([row[j+1], ping_range])
				# print("corr matrix: ", correlation_matrix)
		...
		for row in rows:
			...
			for j in range(4, len(row), 2):
				...
				data_row = row[:4] + row[j: j + 2] + row[-8:]
				# print(data_row)
	...
	if ("controlled" in sys.argv[1]):
		...
		for i in range(len(controlled_rows)):
			...
			for j in range(4, len(row), 2):
				...
				r = [app, version]
				r.extend([test_name, latency, rx, tx, n_tcp, n_udp, ping_median_google, ping_median_amazon, ping_median_mobikwik])
		...
		with open(filename, 'w', newline='') as csvfile:
			csvwriter = csv.writer(csvfile)
			csvwriter.writerow(['APP', 'VERSION', 'FEATURE_NAME', 'LATENCY', 'RX_BYTES (Mb)', 'TX_BYTES (Mb)', '#TCP_CONNECTIONS', '#UDP_CONNECTIONS', 'PING_MEDIAN_GOOGLE', 'PING_MEDIAN_AMAZON', 'PING_MEDIAN_MOBIKWIK'])
	...
	if (sys.argv[1].split("-")[0] == "jumbo" or sys.argv[1].split("-")[0] == "all"):
		...
		fields = ["FEATURE", "STUDENT", "ANDROID", "DISTANCE", "RAM", "LATENCY", "TIME_OF_DAY", "NET_TYPE", "PING_MEDIAN_GOOGLE", "PING_MEDIAN_AMAZON", "PING_MEDIAN_MOBIKWIK","LATITUDE","LONGITUDE", "ALTITUDE"]
